...
import com.digital4.service.PrepurchaseService;
...
public class PrepurchaseController {
	...
	public ResponseEntity<?> purchaseDetail(
		...
		try {
			...
			prepurchase_res = prepurchaseSvc.addPurchaseDetail(prepurchase, personId, token);
		} catch (Exception e) {
	...
	public ResponseEntity<?> getPrepurchase(@ApiParam(value = "prepurchaseId") @PathVariable String keyword, HttpServletRequest req) {
		...
		try {
			String token = req.getHeader("Authorization");
			Prepurchase prepurchase = prepurchaseSvc.getPrepurchase(keyword, token); 
	...
	public ResponseEntity<?> getPurchaseHistory(HttpServletRequest req) {
		...
		try {
			...
			List<Prepurchase> prepurchaseList = prepurchaseSvc.getAllPrepurchase(personId, token);
